...
GTitle.prototype.draw = function() {
	...
	fill(this.fontColor);
	noStroke();
	textAlign(this.textAlignment, BOTTOM);
	text(this.text, this.plotPos, -this.offset - this.dim[1]);
...
function GAxisLabel(parent, type, dim) {
	...
	// General properties
	this.type = (type === BOTTOM || type === TOP || type === LEFT || type === RIGHT) ? type : BOTTOM;
	this.dim = dim.slice();
	this.relativePos = 0.5;
	this.plotPos = (this.type === BOTTOM || this.type === TOP) ? this.relativePos * this.dim[0] : -this.relativePos * this.dim[1];
	this.offset = 35;
	this.rotate = (this.type === BOTTOM || this.type === TOP) ? false : true;
...
GAxisLabel.prototype.drawAsXLabel = function() {
	...
	if (this.rotate) {
		...
		translate(this.plotPos, this.offset);
		rotate(-0.5 * Math.PI);
		text(this.text, 0, 0);
	} else {
		textAlign(this.textAlignment, TOP);
		text(this.text, this.plotPos, this.offset);
	}
...
GAxisLabel.prototype.drawAsYLabel = function() {
	...
	if (this.rotate) {
		textAlign(this.textAlignment, BOTTOM);
		translate(-this.offset, this.plotPos);
		rotate(-0.5 * Math.PI);
		text(this.text, 0, 0);
	} else {
		textAlign(RIGHT, CENTER);
		text(this.text, -this.offset, this.plotPos);
	}
...
GAxisLabel.prototype.drawAsTopLabel = function() {
	...
	if (this.rotate) {
		...
		translate(this.plotPos, -this.offset - this.dim[1]);
		rotate(-0.5 * Math.PI);
		text(this.text, 0, 0);
	} else {
		textAlign(this.textAlignment, BOTTOM);
...
GAxisLabel.prototype.drawAsRightLabel = function() {
	...
	if (this.rotate) {
		textAlign(this.textAlignment, TOP);
		translate(this.offset + this.dim[0], this.plotPos);
		rotate(-0.5 * Math.PI);
		text(this.text, 0, 0);
	} else {
		textAlign(LEFT, CENTER);
		text(this.text, this.offset + this.dim[0], this.plotPos);
	}
...
function GAxis(parent, type, dim, lim, log) {
	...
	this.rotateTickLabels = (this.type === BOTTOM || this.type === TOP) ? false : true;
	this.drawTickLabels = (this.type === BOTTOM || this.type === LEFT) ? true : false;
...
GAxis.prototype.drawAsXAxis = function() {
	...
	line(0, this.offset, this.dim[0], this.offset);

	for ( i = 0; i < this.plotTicks.length; i++) {
		if (this.ticksInside[i]) {
			if (this.log && this.tickLabels[i] === "") {
				line(this.plotTicks[i], this.offset, this.plotTicks[i], this.offset + this.smallTickLength);
			} else {
				line(this.plotTicks[i], this.offset, this.plotTicks[i], this.offset + this.tickLength);
			}
		}
	}
		...
		if (this.rotateTickLabels) {
			...
			for ( i = 0; i < this.plotTicks.length; i++) {
				if (this.ticksInside[i] && this.tickLabels[i] !== "") {
					push();
					translate(this.plotTicks[i], this.offset + this.tickLabelOffset);
					rotate(-halfPI);
					text(this.tickLabels[i], 0, 0);
					pop();
				}
			}
		} else {
			...
			for ( i = 0; i < this.plotTicks.length; i++) {
				if (this.ticksInside[i] && this.tickLabels[i] !== "") {
					text(this.tickLabels[i], this.plotTicks[i], this.offset + this.tickLabelOffset);
				}
			}
		}
...
GAxis.prototype.drawAsYAxis = function() {
	...
	line(-this.offset, 0, -this.offset, -this.dim[1]);

	for ( i = 0; i < this.plotTicks.length; i++) {
		if (this.ticksInside[i]) {
			if (this.log && this.tickLabels[i] === "") {
				line(-this.offset, this.plotTicks[i], -this.offset - this.smallTickLength, this.plotTicks[i]);
			} else {
				line(-this.offset, this.plotTicks[i], -this.offset - this.tickLength, this.plotTicks[i]);
			}
		}
	}
	...
	if (this.drawTickLabels) {
		...
		if (this.rotateTickLabels) {
			...
			for ( i = 0; i < this.plotTicks.length; i++) {
				if (this.ticksInside[i] && this.tickLabels[i] !== "") {
					push();
					translate(-this.offset - this.tickLabelOffset, this.plotTicks[i]);
					rotate(-halfPI);
					text(this.tickLabels[i], 0, 0);
					pop();
				}
			...
			for ( i = 0; i < this.plotTicks.length; i++) {
				if (this.ticksInside[i] && this.tickLabels[i] !== "") {
					text(this.tickLabels[i], -this.offset - this.tickLabelOffset, this.plotTicks[i]);
				}
			}
		}
...
function GHistogram(parent, type, dim, plotPoints) {
	...
	this.visible = true;
	this.separations = [2];
	this.bgColors = [color(150, 150, 255)];
	this.lineColors = [color(100, 100, 255)];
	this.lineWidths = [1];
	this.differences = [];
	this.leftSides = [];
	this.rightSides = [];
	...
	if (this.visible) {
		...
		for (var i = 0; i < nPoints; i++) {
			...
			if (point.isValid()) {
				...
				fill(this.bgColors[i % this.bgColors.length]);
		...
		if (this.rotateLabels) {
			...
			for ( i = 0; i < nPoints; i++) {
				...
				if (point.isValid() && point.getX() >= 0 && point.getX() <= this.dim[0]) {
					push();
					translate(point.getX(), this.labelsOffset);
					rotate(-halfPI);
					text(point.getLabel(), 0, 0);
					pop();
				}
			}
		} else {
			...
			for ( i = 0; i < nPoints; i++) {
				...
				if (point.isValid() && point.getX() >= 0 && point.getX() <= this.dim[0]) {
					text(point.getLabel(), point.getX(), this.labelsOffset);
				}
			}
		}
	} else {
		if (this.rotateLabels) {
			...
			for ( i = 0; i < nPoints; i++) {
				...
				if (point.isValid() && -point.getY() >= 0 && -point.getY() <= this.dim[1]) {
					push();
					translate(-this.labelsOffset, point.getY());
					rotate(-halfPI);
					text(point.getLabel(), 0, 0);
					pop();
				}
			...
			for ( i = 0; i < nPoints; i++) {
				...
				if (point.isValid() && -point.getY() >= 0 && -point.getY() <= this.dim[1]) {
					text(point.getLabel(), -this.labelsOffset, point.getY());
				}
			}
		}
	}
...
function GLayer(parent, id, dim, xLim, yLim, xLog, yLog) {
	...
	this.plotPoints = [];
	this.inside = [];
	this.pointColors = [color(255, 0, 0, 150)];
	this.pointSizes = [7];

	// Line properties
	this.lineColor = color(0, 150);
	this.lineWidth = 1;
	...
	// Labels properties
	this.labelBgColor = color(255, 200);
...
GLayer.prototype.drawPoints = function() {
	...
	if (arguments.length === 0) {
		...
		if (nColors === 1 && nSizes === 1) {
			fill(this.pointColors[0]);

			for ( i = 0; i < nPoints; i++) {
				if (this.inside[i]) {
					ellipse(this.plotPoints[i].getX(), this.plotPoints[i].getY(), this.pointSizes[0], this.pointSizes[0]);
				}
			}
		} else if (nColors === 1) {
			fill(this.pointColors[0]);
			...
			for ( i = 0; i < nPoints; i++) {
				if (this.inside[i]) {
					ellipse(this.plotPoints[i].getX(), this.plotPoints[i].getY(), this.pointSizes[i % nSizes], this.pointSizes[i % nSizes]);
				}
			}
		} else if (nSizes === 1) {
			for ( i = 0; i < nPoints; i++) {
				if (this.inside[i]) {
					fill(this.pointColors[i % nColors]);
					ellipse(this.plotPoints[i].getX(), this.plotPoints[i].getY(), this.pointSizes[0], this.pointSizes[0]);
				}
			}
		} else {
		...
		for ( i = 0; i < nPoints; i++) {
			if (this.inside[i]) {
				image(arguments[0], this.plotPoints[i].getX(), this.plotPoints[i].getY());
			}
		}
...
GLayer.prototype.drawPoint = function() {
	...
	if (this.isInside(xPlot, yPlot)) {
		...
		if ( typeof pointImg !== "undefined") {
			imageMode(CENTER);
			image(pointImg, xPlot, yPlot);
		} else {
			ellipseMode(CENTER);
			fill(pointColor);
			noStroke();
			ellipse(xPlot, yPlot, pointSize, pointSize);
		}
...
GLayer.prototype.drawLines = function() {
	...
	for (var i = 0; i < nPoints - 1; i++) {
		if (this.inside[i] && this.inside[i + 1]) {
			line(this.plotPoints[i].getX(), this.plotPoints[i].getY(), this.plotPoints[i + 1].getX(), this.plotPoints[i + 1].getY());
			...
			if (this.inside[i]) {
				line(this.plotPoints[i].getX(), this.plotPoints[i].getY(), this.cuts[0][0], this.cuts[0][1]);
			} else if (this.inside[i + 1]) {
				line(this.cuts[0][0], this.cuts[0][1], this.plotPoints[i + 1].getX(), this.plotPoints[i + 1].getY());
			} else if (nCuts >= 2) {
				line(this.cuts[0][0], this.cuts[0][1], this.cuts[1][0], this.cuts[1][1]);
			}
		}
	}
...
GLayer.prototype.drawLine = function() {
	...
	if (plotPoint1.isValid() && plotPoint2.isValid()) {
		...
		if (inside1 && inside2) {
			line(plotPoint1.getX(), plotPoint1.getY(), plotPoint2.getX(), plotPoint2.getY());
		} else {
			// At least one of the points is outside the inner region.
			// Obtain the valid line box intersections
			...
			if (inside1) {
				line(plotPoint1.getX(), plotPoint1.getY(), this.cuts[0][0], this.cuts[0][1]);
			} else if (inside2) {
				line(this.cuts[0][0], this.cuts[0][1], plotPoint2.getX(), plotPoint2.getY());
			} else if (nCuts >= 2) {
				line(this.cuts[0][0], this.cuts[0][1], this.cuts[1][0], this.cuts[1][1]);
			}
		}
	...
	if ( typeof shapePoints !== "undefined" && shapePoints.length > 0) {
		...
		for (var i = 0; i < shapePoints.length; i++) {
			if (shapePoints[i].isValid()) {
				vertex(shapePoints[i].getX(), shapePoints[i].getY());
			}
		}
...
GLayer.prototype.drawLabel = function(point) {
	...
	if (this.isValidNumber(xPlot) && this.isValidNumber(yPlot)) {
		...
		// Draw the background
		fill(this.labelBgColor);
		rect(xLabelPos - delta, yLabelPos - this.fontSize - delta, textWidth(point.getLabel()) + 2 * delta, this.fontSize + 2 * delta);

		// Draw the text
		fill(this.fontColor);
		text(point.getLabel(), xLabelPos, yLabelPos);
		pop();
	}
};
	...
	if (polygonPoints.length > 2) {
		...
		if (croppedPoly.length > 2) {
			...
			for ( i = 0; i < croppedPoly.length; i++) {
				vertex(croppedPoly[i].getX(), croppedPoly[i].getY());
			}
...
function GPlot() {
	...
	this.boxBgColor = color(245);
	this.boxLineColor = color(210);
	this.boxLineWidth = 1;
	this.gridLineColor = color(210);
	...
	// Axes and title
	this.xAxis = new GAxis(this.parent, BOTTOM, this.dim, this.xLim, this.xLog);
	this.topAxis = new GAxis(this.parent, TOP, this.dim, this.xLim, this.xLog);
	this.yAxis = new GAxis(this.parent, LEFT, this.dim, this.yLim, this.yLog);
	this.rightAxis = new GAxis(this.parent, RIGHT, this.dim, this.yLim, this.yLog);
	this.title = new GTitle(this.parent, this.dim);

	// Setup for the mouse events
	this.zoomingIsActive = false;
	this.zoomFactor = 1.3;
	this.increaseZoomButton = LEFT;
	this.decreaseZoomButton = RIGHT;
	this.increaseZoomKeyModifier = GPlot.NONE;
	this.decreaseZoomKeyModifier = GPlot.NONE;
	this.centeringIsActive = false;
	this.centeringButton = LEFT;
	this.centeringKeyModifier = GPlot.NONE;
	this.panningIsActive = false;
	this.panningButton = LEFT;
...
GPlot.prototype.beginDraw = function() {
	push();
	translate(this.pos[0] + this.mar[1], this.pos[1] + this.mar[2] + this.dim[1]);
};
...
GPlot.prototype.drawBackground = function() {
	...
	fill(this.bgColor);
	noStroke();
	rect(-this.mar[1], -this.mar[2] - this.dim[1], this.outerDim[0], this.outerDim[1]);
	pop();
};
...
GPlot.prototype.drawBox = function() {
	push();
	rectMode(CORNER);
	fill(this.boxBgColor);
	stroke(this.boxLineColor);
	strokeWeight(this.boxLineWidth);
	strokeCap(SQUARE);
	rect(0, -this.dim[1], this.dim[0], this.dim[1]);
	pop();
};
	...
	if (gridType === GPlot.BOTH || gridType === GPlot.VERTICAL) {
		...
		for (i = 0; i < xPlotTicks.length; i++) {
			if (xPlotTicks[i] >= 0 && xPlotTicks[i] <= this.dim[0]) {
				line(xPlotTicks[i], 0, xPlotTicks[i], -this.dim[1]);
			}
		}
	}
	...
	if (gridType === GPlot.BOTH || gridType === GPlot.HORIZONTAL) {
		...
		for (i = 0; i < yPlotTicks.length; i++) {
			if (-yPlotTicks[i] >= 0 && -yPlotTicks[i] <= this.dim[1]) {
				line(0, yPlotTicks[i], this.dim[0], yPlotTicks[i]);
			}
		}
	}
...
GPlot.prototype.drawLegend = function(text, xRelativePos, yRelativePos) {
	...
	for (var i = 0; i < text.length; i++) {
		...
		if (i === 0) {
			fill(this.mainLayer.getLineColor());
			rect(plotPosition[0], plotPosition[1], rectSize, rectSize);
			this.mainLayer.drawAnnotation(text[i], position[0], position[1], LEFT, CENTER);
		} else {
			fill(this.layerList[i - 1].getLineColor());
			rect(plotPosition[0], plotPosition[1], rectSize, rectSize);
			this.layerList[i - i].drawAnnotation(text[i], position[0], position[1], LEFT, CENTER);
		}
	}
...
GPlot.prototype.activateZooming = function() {
	...
	if (increaseKeyModifier === SHIFT || increaseKeyModifier === CONTROL || increaseKeyModifier === ALT || increaseKeyModifier === GPlot.NONE) {
		this.increaseZoomKeyModifier = increaseKeyModifier;
	}

	if (decreaseKeyModifier === SHIFT || decreaseKeyModifier === CONTROL || decreaseKeyModifier === ALT || decreaseKeyModifier === GPlot.NONE) {
...
GPlot.prototype.activateCentering = function() {
	...
	if (keyModifier === SHIFT || keyModifier === CONTROL || keyModifier === ALT || keyModifier === GPlot.NONE) {
	...
	if (this.isOverBox()) {
		...
		if (this.zoomingIsActive) {
			if (button === this.increaseZoomButton && modifier === this.increaseZoomKeyModifier) {
				...
				this.zoom(this.zoomFactor, mouseX, mouseY);
				...
				this.zoom(1 / this.zoomFactor, mouseX, mouseY);
			}
		}
...
GPlot.prototype.mouseDownEvent = function(event) {
	...
	if (this.isOverBox()) {
		...
		if (this.panningIsActive && button === this.panningButton && modifier === this.panningKeyModifier) {
			...
			this.panningReferencePoint = this.getValueAt(mouseX, mouseY);
		}
...
GPlot.prototype.mouseMoveEvent = function(event) {
	...
	if (this.panningIsActive && button === this.panningButton && modifier === this.panningKeyModifier) {
		this.align(this.panningReferencePoint, mouseX, mouseY);
	}
...
GPlot.prototype.touchStartEvent = function(event) {
	...
	if (this.isOverBox()) {
		...
		if(this.zoomingIsActive && typeof e.touches !== "undefined" && e.touches.length === 2){
			...
			this.zoomStartPosition = [mouseX, mouseY];
		}
